...
class Deploy
{
	...
	public function push()
	{
		...
		foreach($this->hosts AS $key => $row)
		{
			...
			{
				echo exec("ssh -p $this->ssh_port $row 'cd $this->remote_dir && git pull origin $this->branch && php artisan migrate && composer.phar update'") . "\n\n";
			} else
			{
				echo exec("ssh -p $this->ssh_port $row 'cd $this->remote_dir && git pull origin $this->branch && composer.phar update'") . "\n\n";				
			}
		}
